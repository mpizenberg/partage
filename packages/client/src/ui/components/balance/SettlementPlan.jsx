...
export const SettlementPlan = (props) => {
    const { addTransfer, identity } = useAppContext();
    const [isSettling, setIsSettling] = createSignal(null);
    ...
    const isUserInvolved = (fromId, toId) => {
        const userId = identity()?.publicKeyHash;
        return userId === fromId || userId === toId;
    };
    return (<Show when={props.plan.transactions.length > 0} fallback={<div class="settlement-empty">
      ...
      <div class="settlement-plan">
        ...
        <div class="settlement-list">
          <For each={props.plan.transactions}>
            ...
            return (<div class={`settlement-item ${involved ? 'settlement-item-involved' : ''}`}>
                  ...
                  <Show when={involved}>
                    <Button variant="primary" size="small" onClick={() => handleSettle(transaction.from, transaction.to, transaction.amount)} disabled={loading}>
