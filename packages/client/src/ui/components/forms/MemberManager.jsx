...
export const MemberManager = (props) => {
    ...
    return (<div class="member-manager">
      ...
      <div class="member-list mb-md">
        <For each={props.members}>
          {(member) => (<div class="member-item">
              ...
              <Show when={member.id !== props.currentUserId}>
                <button type="button" class="member-remove" onClick={() => props.onRemoveMember(member.id)} aria-label={`Remove ${member.name}`}>
                  ×
                </button>
              </Show>
            </div>)}
      ...
      <div class="add-member-form">
        ...
        <div class="flex gap-sm">
          <Input id="new-member-name" type="text" value={newMemberName()} placeholder="Enter name..." onInput={(e) => {
            setNewMemberName(e.currentTarget.value);
            setError(null);
        }} onKeyPress={handleKeyDown} error={!!error()}/>
          <Button type="button" variant="secondary" onClick={handleAddMember} disabled={!newMemberName().trim()}>
            Add
          </Button>
        </div>
        <Show when={error()}>
