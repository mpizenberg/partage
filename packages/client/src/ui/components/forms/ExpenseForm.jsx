...
export const ExpenseForm = (props) => {
    ...
    const [selectedBeneficiaries, setSelectedBeneficiaries] = createSignal(new Set([identity()?.publicKeyHash || '']));
    const [splitType, setSplitType] = createSignal('shares');
    const [beneficiaryShares, setBeneficiaryShares] = createSignal(new Map());
    const [beneficiaryAmounts, setBeneficiaryAmounts] = createSignal(new Map());
    ...
    const updateBeneficiaryShares = (memberId, delta) => {
    ...
    const updateBeneficiaryAmount = (memberId, value) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const beneficiaries = Array.from(selectedBeneficiaries()).map((memberId) => {
                if (splitType() === 'exact') {
                    return {
                        memberId,
                        splitType: 'exact',
    ...
    return (<form class="expense-form" onSubmit={handleSubmit}>
      {/* Basic Section */}
      <div class="form-section">
        <div class="form-field">
          <label class="form-label">Description</label>
          <Input type="text" value={description()} placeholder="What was this expense for?" disabled={isSubmitting()} error={!!errors().description} onInput={(e) => setDescription(e.currentTarget.value)}/>
        </div>
      ...
      <div class="form-section">
        ...
        <Select value={payerId()} disabled={isSubmitting()} error={errors().payer} onChange={(e) => setPayerId(e.currentTarget.value)}>
      ...
      <div class="form-section">
        <div class="form-section-header">
          ...
          <div class="split-type-toggle">
            <button type="button" class={`toggle-btn ${splitType() === 'shares' ? 'active' : ''}`} onClick={() => setSplitType('shares')} disabled={isSubmitting()}>
              Shares
            </button>
            <button type="button" class={`toggle-btn ${splitType() === 'exact' ? 'active' : ''}`} onClick={() => setSplitType('exact')} disabled={isSubmitting()}>
              Exact
            </button>
          </div>
        </div>
          ...
          <For each={members()}>
            ...
            return (<div class={`beneficiary-item ${isSelected() ? 'selected' : ''}`}>
                  <label class="beneficiary-checkbox">
                    <input type="checkbox" checked={isSelected()} disabled={isSubmitting()} onChange={() => toggleBeneficiary(member.id)}/>
                  ...
                  <Show when={isSelected()}>
                    <Show when={splitType() === 'shares'}>
                      <div class="beneficiary-control">
                        <button type="button" class="control-btn" onClick={() => updateBeneficiaryShares(member.id, -1)} disabled={isSubmitting()}>
                          −
                        </button>
                    ...
                    <Show when={splitType() === 'exact'}>
                      <div class="beneficiary-control">
                        <Input type="number" value={(beneficiaryAmounts().get(member.id) || 0).toString()} placeholder="0.00" step={0.01} min={0} disabled={isSubmitting()} onInput={(e) => updateBeneficiaryAmount(member.id, e.currentTarget.value)}/>
                      </div>
                    </Show>
                  </Show>
                </div>);
      ...
      {/* Advanced Section */}
      <div class="form-section">
        <button type="button" class="form-toggle-btn" onClick={() => setShowAdvanced(!showAdvanced())}>
        ...
        <Show when={showAdvanced()}>
          <div class="form-field">
            ...
            <Select value={category()} disabled={isSubmitting()} onChange={(e) => setCategory(e.currentTarget.value)}>
              <option value="">Select category</option>
              <For each={CATEGORIES}>
                {(cat) => (<option value={cat.value}>
                    {cat.emoji} {cat.label}
                  </option>)}
              </For>
            </Select>
          ...
          <div class="form-field">
            ...
            <Input type="text" value={location()} placeholder="Where was this?" disabled={isSubmitting()} onInput={(e) => setLocation(e.currentTarget.value)}/>
          </div>

          <div class="form-field">
            <label class="form-label">Notes</label>
            <textarea class="form-textarea" value={notes()} placeholder="Add any additional notes..." rows={3} disabled={isSubmitting()} onInput={(e) => setNotes(e.currentTarget.value)}/>
          </div>
        </Show>
      </div>
