...
import { getDB } from '../../core/storage/indexeddb';
...
import { generateKeypair, exportKeypair, generateSigningKeypair, exportSigningKeypair, generateSymmetricKey, exportSymmetricKey, importSymmetricKey, } from '../../core/crypto';
// Create context
...
export const AppProvider = (props) => {
    ...
    const [loroStore, setLoroStore] = createSignal(null);
    // Derived state
    const [entries, setEntries] = createSignal([]);
    const [balances, setBalances] = createSignal(new Map());
    // UI state
    ...
    // Create new group
    const createGroup = async (name, currency, virtualMembers) => {
        try {
            setIsLoading(true);
            setError(null);
    ...
    const refreshEntries = async (groupId, keyVersion) => {
        try {
