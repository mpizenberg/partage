...
export const GroupSelectionScreen = () => {
    ...
    return (<Show when={!showCreateGroup()} fallback={<CreateGroupScreen onCancel={() => setShowCreateGroup(false)}/>}>
      <div class="container">
        <div class="group-selection-screen" style="max-width: 600px; margin: 0 auto; padding-top: var(--space-xl);">
          ...
          {/* Group list */}
          <Show when={groups().length > 0} fallback={<div class="empty-state mb-xl">
            ...
            <div class="group-list mb-xl">
              <For each={groups()}>
                {(group) => (<div class="card clickable group-card" onClick={() => handleSelectGroup(group.id)}>
          ...
          {/* Create group button */}
          <Button variant="primary" size="large" onClick={() => setShowCreateGroup(true)} class="w-full">
            + Create New Group
          </Button>
        </div>
      </div>
