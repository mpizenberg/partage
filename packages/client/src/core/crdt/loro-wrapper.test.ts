...
import type { ExpenseEntry, TransferEntry } from '@partage/shared';

describe('LoroEntryStore', () => {
  let store: LoroEntryStore;
  ...
  describe('Entry Creation', () => {
    it('should create an expense entry', async () => {
      ...
      expect((retrieved as ExpenseEntry)?.description).toBe('Dinner');
      expect((retrieved as ExpenseEntry)?.amount).toBe(100);
    });
    ...
    it('should create a transfer entry', async () => {
      ...
      expect((retrieved as TransferEntry)?.from).toBe('member-1');
      expect((retrieved as TransferEntry)?.to).toBe('member-2');
    });
  ...
  describe('Entry Modification', () => {
    it('should create a new version when modifying an entry', async () => {
      ...
      expect((retrieved as ExpenseEntry)?.description).toBe('Modified');
  ...
  describe('Snapshot and Sync', () => {
    it('should export and import snapshots', async () => {
      ...
      expect(retrieved).toBeDefined();
      expect((retrieved as ExpenseEntry)?.description).toBe('Snapshot test');
    });
    ...
    it('should apply updates from another store', async () => {
      ...
      expect(retrieved).toBeDefined();
      expect((retrieved as ExpenseEntry)?.description).toBe('Sync test');
    });
  });
