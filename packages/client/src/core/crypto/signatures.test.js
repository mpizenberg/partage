...
import { generateSigningKeypair, sign, verify, exportSigningKeypair, importSigningKeypair, importVerificationKey, signString, verifyString, signJSON, verifyJSON, } from './signatures';
describe('Digital Signatures (ECDSA)', () => {
    ...
    describe('Security Properties', () => {
        it('should not allow signing with public key', async () => {
            const keypair = await generateSigningKeypair();
            const data = new TextEncoder().encode('test');
            await expect(sign(data, keypair.publicKey)).rejects.toThrow();
        });
        it('should not allow verifying without public key', async () => {
            ...
            const isValid = await verify(data, signature, keypair.privateKey);
            expect(isValid).toBe(false);
        });
